class Budget < ActiveRecord::Base
  ...
  def summary_for_phase(phase)
    if phases.exists? && phases.send(phase).summary.present?
      phases.send(phase).summary
    else
      send("summary_#{phase}").try(:html_safe)
    end
  end
