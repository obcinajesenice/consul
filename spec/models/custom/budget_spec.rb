...
describe Budget do
  ...
  describe "summary" do
    describe "Without Budget::Phase associated" do
      ...
      it "changes depending on the phase, falling back to budget summary attributes" do
        Budget::Phase::PHASE_KINDS.each do |phase_kind|
          budget.phase = phase_kind
          expect(budget.summary).to eq(budget.send("summary_#{phase_kind}"))
          expect(budget.summary).to be_html_safe
        end
      end
    end
    ...
    describe "With associated Budget::Phases" do
      before do
        budget.phases.each do |phase|
          phase.summary = phase.kind.humanize
          phase.save
        end
      end
      ...
      it "changes depending on the phase" do
        Budget::Phase::PHASE_KINDS.each do |phase_kind|
          budget.phase = phase_kind
          expect(budget.summary).to eq(phase_kind.humanize)
        end
      end
    end
  end
